...
namespace FramingsectorOntology
{
	...
	public class FramingSector : GnossOCBase
	{
		...
		public FramingSector(SemanticEntityModel pSemCmsModel, LanguageEnum idiomaUsuario) : base()
		{
		...
		public virtual SecondaryResource ToGnossApiResource(ResourceApi resourceAPI, string identificador)
		{
			...
			GetProperties();
			SecondaryOntology ontology = new SecondaryOntology(resourceAPI.GraphsUrl, resourceAPI.OntologyUrl, "http://w3id.org/roh/FramingSector", "http://w3id.org/roh/FramingSector", prefList, propList, identificador, listSecondaryEntity, null);
		...
		public override List<string> ToOntologyGnossTriples(ResourceApi resourceAPI)
		{
			...
			AgregarTripleALista($"{resourceAPI.GraphsUrl}items/FramingSector_{ResourceID}_{ArticleID}", "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", $"<http://w3id.org/roh/FramingSector>", list, " . ");
			AgregarTripleALista($"{resourceAPI.GraphsUrl}items/FramingSector_{ResourceID}_{ArticleID}", "http://www.w3.org/2000/01/rdf-schema#label", $"\"http://w3id.org/roh/FramingSector\"", list, " . ");
			AgregarTripleALista($"{resourceAPI.GraphsUrl}{ResourceID}", "http://gnoss/hasEntidad", $"<{resourceAPI.GraphsUrl}items/FramingSector_{ResourceID}_{ArticleID}>", list, " . ");
			if (this.Dc_title != null)
			{
				foreach (LanguageEnum idioma in this.Dc_title.Keys)
				{
					AgregarTripleALista($"{resourceAPI.GraphsUrl}items/FramingSector_{ResourceID}_{ArticleID}", "http://purl.org/dc/elements/1.1/title", $"\"{GenerarTextoSinSaltoDeLinea(this.Dc_title[idioma])}\"", list, $"{idioma} . ");
				}
			}
			if (this.Dc_identifier != null)
			{
				AgregarTripleALista($"{resourceAPI.GraphsUrl}items/FramingSector_{ResourceID}_{ArticleID}", "http://purl.org/dc/elements/1.1/identifier", $"\"{GenerarTextoSinSaltoDeLinea(this.Dc_identifier)}\"", list, " . ");
			}
			return list;
		}
