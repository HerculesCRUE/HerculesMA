...
namespace ContributionGradeOntology
{
	public class ContributionGrade : GnossOCBase
	{
		...
		public ContributionGrade() : base() { } 

		public ContributionGrade(SemanticEntityModel pSemCmsModel, LanguageEnum idiomaUsuario) : base()
		{
		...
		[LABEL(LanguageEnum.es,"Identificador del grado de contribución")]
		...
		public virtual SecondaryResource ToGnossApiResource(ResourceApi resourceAPI,string identificador)
		{
			...
			GetProperties();
			SecondaryOntology ontology = new SecondaryOntology(resourceAPI.GraphsUrl, resourceAPI.OntologyUrl, "http://w3id.org/roh/ContributionGrade", "http://w3id.org/roh/ContributionGrade", prefList, propList,identificador,listSecondaryEntity, null);
		...
		public override List<string> ToOntologyGnossTriples(ResourceApi resourceAPI)
		{
			...
			list.Add($"<{resourceAPI.GraphsUrl}items/ContributionGrade_{ResourceID}_{ArticleID}> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://w3id.org/roh/ContributionGrade> . ");
			list.Add($"<{resourceAPI.GraphsUrl}items/ContributionGrade_{ResourceID}_{ArticleID}> <http://www.w3.org/2000/01/rdf-schema#label> \"http://w3id.org/roh/ContributionGrade\" . ");
			list.Add($"<{resourceAPI.GraphsUrl}{ResourceID}> <http://gnoss/hasEntidad> <{resourceAPI.GraphsUrl}items/ContributionGrade_{ResourceID}_{ArticleID}> . ");
				if(this.Dc_title != null)
				{
					foreach (LanguageEnum LanguageEnum in Enum.GetValues(typeof(LanguageEnum)))
					{
						//list.Add($"<{resourceAPI.GraphsUrl}items/ContributionGrade_{ResourceID}_{ArticleID}> <http://purl.org/dc/elements/1.1/title> \"{this.Dc_title.Replace("\r\n", " ").Replace("\n", " ").Replace("\r", " ").Replace("\"", "\\\"")}\" @[LanguageEnum]" .);
					}
				}
				if(this.Dc_identifier != null)
				{
					list.Add($"<{resourceAPI.GraphsUrl}items/ContributionGrade_{ResourceID}_{ArticleID}> <http://purl.org/dc/elements/1.1/identifier> \"{this.Dc_identifier.Replace("\r\n", " ").Replace("\n", " ").Replace("\r", " ").Replace("\"", "\\\"")}\" . ");
				}
			return list;
		}
